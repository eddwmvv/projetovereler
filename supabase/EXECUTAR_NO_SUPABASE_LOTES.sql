-- =============================================
-- SISTEMA DE LOTES - EXECUÇÃO COMPLETA
-- =============================================
-- Execute este script no SQL Editor do Supabase:
-- https://app.supabase.com/project/[seu-projeto]/sql/new
--
-- Este script cria toda a estrutura necessária para o
-- sistema de gerenciamento de lotes de óculos
-- =============================================

-- =============================================
-- 1. CRIAR ENUM PARA STATUS DO LOTE
-- =============================================
DO $$ 
BEGIN
...
CREATE TABLE IF NOT EXISTS public.lote_itens (
  ...
  tamanho TEXT NOT NULL, -- Tamanho do óculos (P, M, G, ou "Geral")
...
-- =============================================
-- 4. CRIAR TABELA DE HISTÓRICO DE LOTES
-- =============================================
...
-- =============================================
-- 6. CRIAR FUNÇÃO PARA GERAR PRÓXIMO NÚMERO DE LOTE
-- =============================================
...
-- =============================================
-- 7. CRIAR FUNÇÃO PARA ATUALIZAR STATUS
-- =============================================
...
-- =============================================
-- 8. CRIAR FUNÇÃO PARA BUSCAR LOTES COM DETALHES
-- =============================================
...
-- =============================================
-- 9. ALTERAR TABELA LOTE_ITENS PARA USAR TAMANHOS
-- =============================================
-- Adicionar coluna tamanho_id e manter tamanho (TEXT) temporariamente
...
DROP POLICY IF EXISTS "Permitir leitura de lotes para usuários autenticados" ON public.lotes;
CREATE POLICY "Permitir leitura de lotes para usuários autenticados"
  ON public.lotes FOR SELECT
  USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Permitir leitura de itens de lote para usuários autenticados" ON public.lote_itens;
CREATE POLICY "Permitir leitura de itens de lote para usuários autenticados"
  ON public.lote_itens FOR SELECT
  USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Permitir leitura de histórico de lotes para usuários autenticados" ON public.lotes_historico;
...
DROP POLICY IF EXISTS "Permitir inserção de lotes para usuários autenticados" ON public.lotes;
CREATE POLICY "Permitir inserção de lotes para usuários autenticados"
  ON public.lotes FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Permitir inserção de itens de lote para usuários autenticados" ON public.lote_itens;
CREATE POLICY "Permitir inserção de itens de lote para usuários autenticados"
  ON public.lote_itens FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Permitir inserção de histórico de lotes para usuários autenticados" ON public.lotes_historico;
...
DROP POLICY IF EXISTS "Permitir atualização de lotes para usuários autenticados" ON public.lotes;
CREATE POLICY "Permitir atualização de lotes para usuários autenticados"
  ON public.lotes FOR UPDATE
  USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Permitir atualização de itens de lote para usuários autenticados" ON public.lote_itens;
CREATE POLICY "Permitir atualização de itens de lote para usuários autenticados"
  ON public.lote_itens FOR UPDATE
  USING (auth.role() = 'authenticated');

-- Políticas para exclusão
DROP POLICY IF EXISTS "Permitir exclusão de lotes para usuários autenticados" ON public.lotes;
CREATE POLICY "Permitir exclusão de lotes para usuários autenticados"
  ON public.lotes FOR DELETE
  USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Permitir exclusão de itens de lote para usuários autenticados" ON public.lote_itens;
...
-- CONCLUÍDO!
-- =============================================
-- A estrutura de lotes foi criada com sucesso.
-- Agora você pode usar a interface web para:
-- 1. Criar novos lotes de óculos
-- 2. Acompanhar o status (Criado → Em Preparo → Recebido)
-- 3. Visualizar detalhes e histórico
-- 4. Gerenciar itens por tamanho
-- =============================================
