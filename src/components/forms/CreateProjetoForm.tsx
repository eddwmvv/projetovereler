...
export function CreateProjetoForm({ open, onOpenChange }: CreateProjetoFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createProjeto.mutateAsync({
        ...
        anoAcao,
        status,
        municipiosIds: municipiosSelecionados.length > 0 ? municipiosSelecionados : undefined,
      });
      setNome('');
      setDescricao('');
      setEmpresaId('');
      ...
      <DialogContent>
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={createProjeto.isPending || !empresaId}>
              {createProjeto.isPending ? 'Criando...' : 'Criar'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
