...
export function HeatmapPerformance({ tipo }: HeatmapPerformanceProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 shadow-sm max-w-full sm:max-w-none w-full">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-2">
        <div>
          <h3 className="text-base sm:text-lg font-semibold text-gray-900">
            Performance por {tipo === 'escola' ? 'Escola' : 'Município'}
          </h3>
          <p className="text-xs sm:text-sm text-gray-500 mt-1">Taxa de conclusão dos alunos</p>
        </div>
      ...
      <div className="space-y-2 sm:space-y-3">
        {dadosHeatmap.slice(0, 8).map((item) => (
          <div
            key={item.id}
            className="flex flex-col xs:flex-row xs:items-center xs:justify-between p-3 sm:p-4 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors gap-2"
          >
            <div className="flex-1 min-w-0">
              <p className="font-medium text-gray-900 truncate text-sm sm:text-base">{item.nome}</p>
              <p className="text-xs text-gray-500 mt-1">
                {item.entregues} de {item.total} alunos
              </p>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 ml-0 sm:ml-4 shrink-0">
              ...
              <span className="text-xs sm:text-sm font-bold text-gray-900 w-8 sm:w-10 text-right">
